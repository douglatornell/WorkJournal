...
* started adding Karyn's `_salinityMatch()` function to `matchData()`
...
* continued adding Karyn's `_salinityMatch()` function to `matchData()`
...
* continued adding Karyn's `_salinityMatch()` function to `matchData()`
  * started adding unit tests; initially, a module generated by the PyCharm AI
...
* started updating Alliance setup section from `graham` to `nibi`
...
* helped Vicente with running Parcels on `nibi`
  * pointed him at the appropriate mesh mask file to use


##### SalishSeaTools

* continued adding Karyn's `_salinityMatch()` function to `matchData()`; PR#159
  * added `test_evaltools_salinity_match` module
    * initially generated by PyCharm AI but I had to do a lot of work to get the tests to pass
      and to replace `unittest` `patch` and `MagicMock` with `pytest` `monkeypatch` mocks
...
* tested doing an actual NEMO run on `fir` using the `pixi` branch
...
* `crop_gribs 12` was delayed ~2h due to 1 unprocessed file
* `collect_river_data SnohomishMonroe` got empty time series


##### NEMO-Cmd

* worked through Pixi setup and NEMO run with Susan on `fir`
...
Vancouver to Juan de Fuca Beach House
...
* added `pixi-sphinx-linkcheck` workflow based on the repo-specific one I created in `NEMO-Cmd`


##### NEMO-Cmd

* continued changing to use Pixi for project & env mgmt; PR#121
  * changed `sphinx-linkcheck` workflow to use new reusable `pixi-sphnx-linkcheck`
...
* `crop_gribs 12` was delayed ~2h due to 1 unprocessed file
* `collect_river_data SquamishBrackendale` got empty time series
* `archive_tarball` failed during `rsync` to `nibi`
  * manual `rsync` started in `tmux` session on `skookum at ~14:52


##### gha-workflows

* added `pixi-pytest-with-coverage` workflow based on the repo-specific one I created in `NEMO-Cmd`
